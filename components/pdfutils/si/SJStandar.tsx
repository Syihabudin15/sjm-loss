...
const generateSJ = (record: IDocument) => {
  const html = `
  <!doctype html>
  <html>
    <head>
    ...
    <div class="page" style="font-size: 12px;">
      ${Header("PENYERAHAN JAMINAN PEMBIAYAAN PENSIUN", record.id, undefined, process.env.NEXT_PUBLIC_APP_LOGO, record.Sumdan.logo)}
      ...
      <div class="my-4">
        ...
        <div class="flex gap-3">
          ...
          <p class="w-4">:</p>
          <p class="flex-1">Penyerahan Jaminan Pembiayaan Pensiun</p>
        </div>
      </div>
      ...
      <div class="mt-2">
        <p>Sehubungan dengan telah diterimanya dana dropping pembiayaan pensiun dari ${record.Sumdan.name} ke rekening Koperasi, maka dari itu sebagai bagian dari administrasi dan realisasi pembiayaan, bersama surat ini kami kirimkan jaminan pembiayaan pensiun atas debitur sebagai berikut :</p>
...
export const printSJStandar = (record: IDocument) => {
  const htmlContent = generateSJ(record);
