    ...
    <div class="page" style="font-size: 12px;">
      ...
      <div class="my-4 flex justify-evenly gap-4 flex-wrap">
        ...
        <div class="font-bold text-blue-500 flex-1 border rounded p-2">
          ...
          <p class="text-lg">Rp. ${IDRFormat(records.reduce((acc, curr) => acc + (["PROSES", "DISETUJUI"].includes(curr.dropping_status) ? curr.plafond : 0), 0))}</p>
          <div class="border-t border-gray-100 my-1"></div>
          <p class="text-lg">NOA ${records.filter((r) => ["PROSES", "DISETUJUI"].includes(r.dropping_status)).length}</p>
        </div>
        <div class="font-bold text-green-500 flex-1 border rounded p-2">
          <p class="opacity-70">DROPPING</p>
          <p class="text-lg">Rp. ${IDRFormat(records.reduce((acc, curr) => acc + (curr.dropping_status === "DISETUJUI" ? curr.plafond : 0), 0))}</p>
          <div class="border-t border-gray-100 my-1"></div>
          <p class="text-lg">NOA ${records.filter((r) => r.dropping_status === "DISETUJUI").length}</p>
        </div>
      </div>
