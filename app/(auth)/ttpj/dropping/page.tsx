...
import { printSJStandar } from "@/components/pdfutils/si/SJStandar";
      ...
      render(value, record, index) {
        return (
          <div className="flex gap-2">
            ...
            <Tooltip title="Berkas Akad & TTPJ">
              <Button
                icon={<FileFilled />}
                size="small"
                onClick={() =>
...
const UpsertDropping = ({
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await fetch("/api/ttpj?id=" + record.id, {
      method: "PUT",
...
const DeleteData = ({
  ...
  const handleDelete = async () => {
    setLoading(true);
    await fetch("/api/ttpj?id=" + data.id, { method: "DELETE" })
      .then((res) => res.json())
      .then(async (res) => {
