...
export default function Page() {
  ...
  const { hasAccess } = useAccess(
    window ? window.location.pathname : "/ttpb/print",
  );
  ...
  const getData = async () => {
    ...
    const res = await fetch(`/api/ttpb/print?${params.toString()}`);
  ...
  const generateNoSI = async () => {
    ...
    await fetch("/api/ttpb/print?id=" + sumdan.id, {
      method: "PATCH",
